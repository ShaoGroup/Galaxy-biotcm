...
die if !open F5,">$ARGV[0]_target_profile.txt";
$title=<F2>;
while(<F2>){
	chomp;
	@temp=split /\t/,$_;
	$score{$temp[0]}=$temp[$ARGV[1]];
}
$title=<F3>;
while(<F3>){
	chomp;
	@temp=split /\t/,$_;
	$druggable{$temp[$ARGV[1]]}="druggable";
}
while(<F4>){
	chomp;
	@temp=split /\t/,$_;
...
while(<F1>){
	chomp;
	@temp=split /\t/,$_;
	print F5 "$temp[$ARGV[1]]\t$symbol{$temp[$ARGV[1]]}\t$score{$temp[$ARGV[1]]}\t";
	if(exists $druggable{$temp[$ARGV[1]]}){
		print F5 "$rank\n";
		$rank++;
	}else{
		print F5 "\n";
